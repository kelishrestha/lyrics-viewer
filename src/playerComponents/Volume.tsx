...
const Volume = ({ value, onChange, isDisabled }: { value: number; onChange: (e: any) => void, isDisabled: boolean }) => {
      ...
      return (
        ...
        >
          <path
            ...
            fill={isDisabled ? disabledColor : volumeColor}
          />
        </svg>
      );
    } else if (volume < 53) {
  ...
  return (
    <div ref={divRef} className="volumeContainer">
      <button onClick={toggleIsOpen} tabIndex={0} aria-label="Volume" disabled={isDisabled}>
        <div className="volume__icon cursor-pointer">{getVolumeSvg(value)}</div>
      </button>
      {isOpen && (
